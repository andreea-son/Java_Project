...
public class UserLoginWindow{
    ...
    public JPanel getPanel(){
        ...
        userLabel = new JLabel("Login as user ");
        ...
        tooltip.setBounds(340, 280 + tooltip.getHeight(), tooltip.getWidth(), tooltip.getHeight());
        panel.add(tooltip);
            ...
            if(ev.getSource() == login) {
                ...
                try {
                    try {
                        ...
                        ResultSet result = statement.executeQuery("SELECT COUNT(*) FROM \"USER\" WHERE USER_EMAIL = '" + email + "'");
                        ...
                        ResultSet result1 = statement1.executeQuery("SELECT * FROM \"USER\" WHERE USER_EMAIL = '" + email + "'");
                        if (result1.next()) {
                            String userPass = result1.getString(4);
                            ...
                            if (numOfLogins == 0) {
                                ...
                                gui.getTabbedPane().add(userChangePasswordWindow.getPanel());
                                gui.getTabbedPane().setSelectedIndex(1);
                                gui.getTabbedPane().remove(panel);
                                numOfLogins = 1;
                                statement2.executeUpdate("UPDATE \"USER\" SET NUM_OF_LOGINS = " + numOfLogins + " WHERE USER_EMAIL = '" + email + "'");
                            }
                            else {
                                numOfLogins++;
                                statement2.executeUpdate("UPDATE \"USER\" SET NUM_OF_LOGINS = " + numOfLogins + " WHERE USER_EMAIL = '" + email + "'");
